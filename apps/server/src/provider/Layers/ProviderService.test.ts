...
describe("agent browser access", () => {
  ...
  it.effect("issues a credential with memory even when browser and device are off", () =>
    Effect.gen(function* () {
      const threadId = asThreadId("thread-memory-always-on");

      const issued = yield* startSessionWith({ browser: false, device: false }, threadId);

      assert.ok(issued[0]?.capabilities.includes("memory"));
      assert.deepEqual(issued, [{ threadId, capabilities: ["memory", "pull-requests"] }]);
    }).pipe(Effect.provide(NodeServices.layer)),
  );
