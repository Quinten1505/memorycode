...
it.effect(
  "always grants pull-requests and memory and gates browser and device access independently",
  () =>
    Effect.gen(function* () {
      ...
      expect(yield* capabilitiesOf(withPreview)).toEqual(["memory", "preview", "pull-requests"]);
      expect(yield* capabilitiesOf(withoutPreview)).toEqual(["memory", "pull-requests"]);
      expect(yield* capabilitiesOf(withDevice)).toEqual(["device", "memory", "pull-requests"]);
    }),
);
